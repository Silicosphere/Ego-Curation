"""Extract top surprising segments from a segment CSV into clip files via ffmpeg.

Usage:
    python -m ego_curation.extract segments/video1_segments.csv \
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("csv", help="Per-video segment CSV produced by --segments-dir")
    parser.add_argument(
        "--top-segments",
        type=int,
        default=None,
...
def extract(
    ...
    if shutil.which(ffmpeg) is None:
        raise SystemExit(f"ffmpeg not found on PATH ('{ffmpeg}'). Install it or pass --ffmpeg.")

    df = pd.read_csv(csv_path).sort_values("surprise", ascending=False).reset_index(drop=True)
    if top_segments:
        df = df.head(top_segments)

    if video is None:
        video = str(df["video"].iloc[0])
    ...
    for row in tqdm(df.itertuples(index=False), desc="Extracting", total=len(df)):
        ...
        out_file = out_dir / f"{video_path.stem}_seg{row.rank:03d}_{start:07.2f}s.mp4"
        ...
        if accurate:
            cmd += ["-ss", f"{start:.3f}", "-c:v", "libx264", "-c:a", "aac"]
        else:
            cmd += ["-c", "copy"]
